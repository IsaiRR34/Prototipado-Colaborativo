...
public class PickupInteractable : MonoBehaviour
{
    ...
    public bool TryCollect(LG_Inventory inventory)
    {
        ...
        if (added)
        {
            ...
            AudioClip clipToPlay = itemData.pickupSound;
#if UNITY_EDITOR
            if (clipToPlay == null)
            {
                ...
                switch (itemData.itemType)
                {
                    case ItemType.Municion:
                        clipToPlay = UnityEditor.AssetDatabase.LoadAssetAtPath<AudioClip>($"{dir}/SFX_Pickup_Ammo.wav");
                        break;
                    case ItemType.LlaveMaestra:
                        clipToPlay = UnityEditor.AssetDatabase.LoadAssetAtPath<AudioClip>($"{dir}/SFX_Pickup_Key.wav");
                        break;
                    default:
                        clipToPlay = UnityEditor.AssetDatabase.LoadAssetAtPath<AudioClip>($"{dir}/SFX_Pickup.wav");
                        break;
                }
            }
#endif
