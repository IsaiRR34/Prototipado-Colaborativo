...
public class LG_Enemy : MonoBehaviour
{
    ...
    [SerializeField] private string footstepSound = "SFX_Enemy_Footstep";
        ...
        if (useProceduralWalk && leftArm != null && rightArm != null)
        {
            ...
            if (isChasing)
            {
                ...
                if (stepTimer <= 0f)
                {
                    ...
                    {
                        // Usamos audio posicional 3D para escuchar si el enemigo está cerca o lejos
                        SoundList.Instance.PlaySoundAtPosition(footstepSound, transform.position);
                    }
